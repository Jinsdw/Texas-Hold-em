...
**最后更新**：2026-09-14 04:25
**当前阶段**：M5 持久化与账号 ✅ 已完成（M2-M5 全部完成）
**整体完成度**：约 90%
...
（无——M2-M5 已全部完成并通过验收，M6 待用户确认启动）

## 已完成任务（最近 10 条）

- [x] 2026-09-14 M5-4 全量验收（78 测试 / 覆盖率 90.27%）与文档收尾

- [x] 2026-09-14 M5-3 筹码持久化与对局记录（端到端落库验证通过）

- [x] 2026-09-14 M5-2 账号系统（scrypt + REST + WS 绑定 + 登录 UI）
...
1. M6 打磨与发布（待确认启动）：LiveKit 语音（可选）→ Tauri 打包（需先安装 Rust 工具链）→ 端到端测试
2. 补验项：安装 Rust 后运行 pnpm tauri dev 验证原生窗口
3. 已知技术债见 PROGRESS 与 docs/ERRORS.md E-004
